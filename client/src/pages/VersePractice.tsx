...
export default function VersePracticePage() {
  ...
  const handleNextVerse = () => {
    ...
    if (nextVerseNum <= verseData.totalVerses) {
      navigate(`/browse/surah/${verseData.surah.id}/${nextVerseNum}`)
    } else {
      // Go back to surah page if no more verses
      navigate(`/browse/surah/${verseData.surah.id}`)
    }
  }
  ...
  const handlePreviousVerse = () => {
    ...
    if (prevVerseNum >= 1) {
      navigate(`/browse/surah/${verseData.surah.id}/${prevVerseNum}`)
    }
  }
        ...
        <div className="flex items-center justify-between mb-6">
          <nav className="text-sm">
            <ol className="flex items-center gap-2 text-gray-500">
              ...
              <li>/</li>
              <li>
                <Link to={`/browse/surah/${surah.id}`} className="hover:text-gray-700">
                  {surah.name}
                </Link>
              </li>
              <li>/</li>
